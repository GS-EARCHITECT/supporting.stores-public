package storeMovementManager.issue.model.repo.cud;
...
@Repository("storeIssueCUDRepo")
public interface StoreIssueCUD_Repo extends CrudRepository<StoreIssueMaster, Long> 
{
@Modifying
...
@Query(value = "SELECT * from  STORE_ISSUE_MASTER where store_request_SEQ_NO in :rids ", nativeQuery = true)
...
@Query(value = "SELECT * from  STORE_ISSUE_MASTER where (store_request_SEQ_NO in :rids and upper(trim(doneflag)) <> 'Y') ", nativeQuery = true)
...
@Query(value = "SELECT * from  STORE_ISSUE_MASTER where (store_request_SEQ_NO in :rids and upper(trim(okflag)) <> 'Y') ", nativeQuery = true)
