...
public interface ConsignmentDetailsRead_Repo extends JpaRepository<ConsignmentDetail, ConsignmentDetailPK> 
{
	...
	Float getTotalSelectConsignmentAssetDetailsPendingForConsignment(@Param("cid") Long cid, @Param("aid") Long rid);
	...
	@Query(value = "SELECT * from  store_Consignment_DETAILS where ((CONSIGNMENT_SEQ_NO = :cid) and (resource_seq_no > 0) and (store_movement_seq_no IS not null or store_movement_seq_no > 0) and upper(trim(doneflag)) <> 'Y') ORDER BY consignment_SEQ_NO;", nativeQuery = true)
	CopyOnWriteArrayList<ConsignmentDetail> getSelectConsignmentResourceDetailsPending(@Param("cid") Long cid);

	@Query(value = "SELECT * from  store_Consignment_DETAILS where ((CONSIGNMENT_SEQ_NO = :cid) and (asset_seq_no > 0) and (store_movement_seq_no IS not null or store_movement_seq_no > 0) and upper(trim(doneflag)) <> 'Y') ORDER BY consignment_SEQ_NO;", nativeQuery = true)
